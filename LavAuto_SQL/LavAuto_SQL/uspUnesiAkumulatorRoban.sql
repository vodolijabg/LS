...
-- Description:	<>
-- =============================================
alter procedure uspUnesiAkumulatorRoban
...
BEGIN TRY
	...
	if (@Amperaza is null) or (@Amperaza = '') 
		raiserror ('Parametar Amperaza je obavezan podatak', 15,1)
	if (@Cena is null) 
...
if ((select COUNT(*) from @Artikal) > 1) --postoji vise artikala izbaci error
	...
	-----------------------------------
	if (not(exists(select * from Opis where Opis = 'Amperaza' and IzvorPodatakaID = @IzvorPodatakaID)))
		begin 
			set @OpisID_Amperaza = (select MAX(Opis_ID)+ 1 from Opis)
			insert into Opis (Opis_ID , Opis, IzvorPodatakaID)
			values (@OpisID_Amperaza, 'Amperaza', @IzvorPodatakaID)
		end
	else
		begin
			set @OpisID_Amperaza = (select top 1 Opis_ID from Opis where Opis = 'Amperaza' and IzvorPodatakaID = @IzvorPodatakaID)
		end
	-----------------------------------
	if (not(exists(select * from Kriterijum where Opis_ID = @OpisID_Amperaza and IzvorPodatakaID = @IzvorPodatakaID)))
		begin 
			...
			values (@Kriterijum_ID, @OpisID_Amperaza, @IzvorPodatakaID)
		end
	else
		begin
			set @Kriterijum_ID = (select top 1 Kriterijum_ID from Kriterijum where Opis_ID = @OpisID_Amperaza and IzvorPodatakaID = @IzvorPodatakaID)
		end
	-----------------------------------	
	if (not(exists(select * from VezaArtikalKriterijum where Artikal_ID = (select top 1 Artikal_ID from @Artikal) and Kriterijum_ID = @Kriterijum_ID and IzvorPodatakaID = @IzvorPodatakaID)))
		begin	 
			...
			values ((select top 1 Artikal_ID from @Artikal), @Kriterijum_ID, @Amperaza, @IzvorPodatakaID)
		end
	else
		begin
			update VezaArtikalKriterijum 
			set Vrednost = @Amperaza  
